...
const REC=[
 {id:'kfr',n:'김치볶음밥',dlc:'base',price:9000,vessel:'plate',time:210,sub:'+ 반숙 계란후라이',
  steps:[['김치·양파·햄 한입 크기로 썰기',()=>cutN('kimchi',1300)>=8],['큰 팬 달구고 식용유 두르기 (160°C↑)',hotPanOil],['파 → 햄·양파 → 김치 순으로 볶기',()=>panWith('kimchi',o=>o.cook>.7)],
    ['간장을 뜨거운 팬 바닥에 눌리기',()=>G.cw.some(c=>c.items.some(o=>o.fire>.01))||(G.soyHotT!==undefined&&G.t-G.soyHotT<90)],['찬밥 넣고 뒤집개로 풀어가며 볶기',()=>G.cw.some(c=>{const g=c.items.filter(o=>o.kind==='grain');return g.length>100&&g.reduce((s,o)=>s+o.stick,0)/g.length<.35;})],
 ...
 {id:'pajeon',n:'파전',dlc:'jeon',price:14000,vessel:'plate',time:220,sub:'쪽파 가득, 오징어 약간',
  steps:[['쪽파를 6~10cm 길이로 썰기',()=>cutN('jjokpa',2000)>=4],['(선택) 오징어 썰기',()=>true],['부침가루 + 물 반죽, 쪽파 넣고 섞기',()=>G.mix.items.some(o=>o.type==='jjokpa')||G.cw.some(c=>c.jeons.some(j=>j.comp.jjokpa))],
 ...
 {id:'buchu',n:'부추전',dlc:'jeon',price:12000,vessel:'plate',time:200,sub:'얇고 바삭하게',
  steps:[['부추를 4~7cm로 썰기',()=>cutN('buchu',2400)>=4],['반죽에 부추(와 양파·당근) 넣고 섞기',()=>G.mix.items.some(o=>o.type==='buchu')||G.cw.some(c=>c.jeons.some(j=>j.comp.buchu))],['기름 넉넉히 두른 팬에 얇게 펴기',()=>G.cw.some(c=>c.jeons.length)],['노릇하게 뒤집어 양면 굽기',()=>G.cw.some(c=>c.jeons.some(j=>j.down===1))],['접시에 담기',()=>G.plate.jeons.length>0]],
