...
function endDay(){if(G.mode!=='career')return;const D=G.day,gas=Math.round(G.gas/10)*10,avg=D.scores.length?Math.round(D.scores.reduce((a,b)=>a+b,0)/D.scores.length):0;
  const rentD=D.n<=3?12000:RENT,wage=typeof staffWage==='function'?staffWage():0;SAVE.money-=rentD+SEASON+gas+wage;const profit=D.rev+D.tips-D.spend-rentD-SEASON-gas-wage;SAVE.day++;SAVE.mid=null;const spoiled=spoilLots();writeSave();G.started=false;setTimeout(()=>checkAch(0),600);
  $('#settleBody').innerHTML=`<div class="st-row"><span>매출</span><b>${won(D.rev)}</b></div><div class="st-row"><span>팁</span><b>${won(D.tips)}</b></div><div class="st-row neg"><span>재료 구입</span><b>${D.spend?'−':''}${won(D.spend)}</b></div>
    <div class="st-row neg"><span>가스비</span><b>${gas?'−':''}${won(gas)}</b></div><div class="st-row neg"><span>임대료${rentD<RENT?' <small>(오픈 할인)</small>':''}</span><b>−${won(rentD)}</b></div><div class="st-row neg"><span>양념·소모품</span><b>−${won(SEASON)}</b></div>${wage?`<div class="st-row neg"><span>인건비</span><b>−${won(wage)}</b></div>`:''}
