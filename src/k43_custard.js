...
function drawEggWater(g,c){const R=c.r,m=clamp(c.mixv||0,0,1),eggs=Math.round(c.egg),vol=c.water+c.egg*50,fr=Math.min(R-6,20+Math.sqrt(vol)*2.1);
  ...
  // whole eggs: glossy whites with a yolk each, drifting round as you stir
...
drawMix=(orig=>function(g,c,x,y){if(!(c.egg>0&&!(c.flour>0)))return orig(g,c,x,y);g.save();g.translate(x,y);drawEggWater(g,c);
