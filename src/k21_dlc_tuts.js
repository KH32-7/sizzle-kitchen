...
Object.assign(TUTS,{
  ...
  katsu:()=>[...tBatter(true),tFridge('loin','돈카츠용 등심'),
    St('등심을 반죽 볼에 넣어요','냉장고에서 꺼낸 등심을 든 채로 반죽 볼을 클릭해요.',[{k:'mix'}],()=>G.mix.items.some(o=>o.type==='loin')||allItems().some(o=>o.type==='loin'&&o.bat),'반죽 볼'),
    tTool('chop','젓가락으로 등심을 집어요 — 튀김옷이 입혀져요.'),St('튀김옷 입은 등심을 도마에','반죽 볼 속 등심을 젓가락으로 집어 도마로 옮겨요 (클릭해서 집고 도마 클릭, 또는 끌어서 놓기).',[{k:'mix'},{k:'board'}],()=>G.board.pieces.some(p=>p.type==='loin'&&p.bat),'도마'),tHand(),
    ...
    tTool('chop','젓가락으로 돈카츠를 건져요.'),St('돈카츠를 도마에','냄비 속 돈카츠를 젓가락으로 집어 도마로 옮겨요 (클릭 후 도마 클릭, 또는 끌어서 놓기).',[{k:'cw',i:0},{k:'board'}],()=>G.board.pieces.some(p=>p.type==='loin'),'건지기'),
