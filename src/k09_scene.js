...
function drawCW(c){const g=ctx;let x=c.x,y=c.y;if(c.tossT>0){const pr=1-c.tossT/.45,o=Math.sin(pr*PI)*18;x-=Math.cos(c.ha)*o;y-=Math.sin(c.ha)*o;}
  ...
  if(!lift){const la=c.kind==='pan'?(c.id===2?2.2:2.5):c.id===0?PI*.78:2.4,lx2=x+Math.cos(la)*(c.r+(c.id===0?20:14)),ly2=y+Math.sin(la)*(c.r+(c.id===0?20:14));g.font='13px "Jua", sans-serif';const nm=c.name,tw=g.measureText(nm).width+16;uiPill(g,lx2-tw/2,ly2-11,tw,23);g.fillStyle='#5a2e22';g.textAlign='center';g.fillText(nm,lx2,ly2+4);}
